...
**Wall clocks**: both clock assets carry a catalog `timeCycle` of 24 dial frames (half hours). `officeState.updateClockSprites()` picks one from `getOfficeDialFraction()` in `sunlight.ts` - the office's own day, so the dials agree with the daylight - and sets `activeTimeSprite`, which wins over every other cycle sprite in `renderScene`. The sun cycle advances regardless of the sunlight toggle, or the clocks would stop. Frames come from `renderer/tools/make-clock-frames.mjs`. A new cycle type must be added to **three** loaders: `src/assetLoader.ts`, `relay/server.mjs` (the relay loads assets itself) and `furnitureCatalog.ts`.
