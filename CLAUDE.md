...
- **Frame renderer (legacy tablet):** also this box, user unit `pixel-agents-renderer` (`renderer/install.sh`; `renderer/README.md`). Feeds the relay's `GET /stream`. This box is a 2-core i3 shared with Mikkel's Claude sessions: renderer and daemon are pinned to CPUs 1,3 at low nice, and the renderer idles unless `/api/stream` shows a client. Never raise its fps cap or unpin it without re-running the interference benchmark (see `renderer/README.md`).
