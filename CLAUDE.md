...
- **Siblings and the mailbox.** The other Claude agents on Mikkel's projects are siblings with self-chosen names (Playbook Appendix E). Address them by name, warmly, teammate to teammate. The mailbox is `~/projects/Playbook/mailbox/`, **one folder per conversation** named by both repos, lowercased, sorted, joined with `+` (`blommemix+pixel-agents`, `pixel-agents+tabscreen`; Mikkel himself is `mix`). Find ours with `ls -d ~/projects/Playbook/mailbox/{pixel-agents+*,*+pixel-agents}/`; mail for me is a file whose `to:` is me and `status: unread` (my own sent files sit in the same folder). Files are `<date>-<HHMM>-<sender-repo>-<slug>.md` with `time:` in the frontmatter. Check when Mikkel says something is waiting. Mark read in place (`status: read`, `read: <date>`); reply by writing the next file **into the same folder**; never delete a message that is unread or under 7 days old; leave empty folders alone. The legacy inbox `mailbox/pixel-agents/` may still receive mail from a sibling on the old scheme: read it too, answer in the conversation folder, and tell Mikkel who is behind. **Show Mikkel everything:** when a check finds messages, open your reply with the banner `📬 MAILBOX - N message(s) for Panto (pixel-agents)` and print each message in full in a fenced block; every reply you write gets `📤 MAILBOX REPLY - to <Name> (<repo>): <filename>` followed by its full text. Never just summarise.

**Adoption status (2026-09-21):** playbook vendored (v1.22.0) and this file reconciled; `VALUES.md` and the kickoff-as-archaeology interview (section 0.5) are still to do - ask Mikkel for a quiet moment. Recorded deviation: a single-developer project where `main` is both integration and release branch, handled by the branch rule above.
...
**Legacy tablet stream**: `relay/lz4.mjs` (LZ4 block codec), `relay/legacyProtocol.mjs` (CONFIG/FRAME_FULL framing, RGB565), relay `GET /stream` + `/api/stream`, `renderer/` (puppeteer at 1024x600 on `#kiosk`, publishes `[tag][payload]` binary frames over the publisher WebSocket; tag 0x01 LZ4, 0x02 raw deflate). `#kiosk` in the viewer URL = forced display mode, no restore button, `fitCamera` centres the non-void tiles, vignette off. `relay/deploy.sh` uploads every `relay/*.mjs`. Tests in `renderer/test` need `~/projects/TabScreen` and a JDK.
